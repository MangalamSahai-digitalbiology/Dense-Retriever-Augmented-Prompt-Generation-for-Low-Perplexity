...
def get_rag(query_id, doc2text, query2docs, top_n, shuffle):
    ...
    docid_list = query2docs[query_id][:top_n]
    ...
    content=[]
    for textid in docid_list:
        content.append(doc2text[str(textid)]) 
...
def main():
    ...
    if args.augmentation_run is not None:
        ...
        for i in range(dataset.num_rows):
            docid_to_text[str(dataset["docid"][i])]=dataset["text"][i]
        ...
        with open(args.augmentation_run, 'r') as h:
            ...
            # import pdb; pdb.set_trace()
            # qid_to_topdocs[h.readline().split(" ")[0]] = int(h.readline().split(" ")[2])
            array = np.genfromtxt(args.augmentation_run, delimiter=" ")
            check = np.split(array[:,2].astype(int), np.unique(array[:, 0], return_index=True)[1][1:])
            for i,docid_list in enumerate(check):
                qid_to_topdocs[i] = docid_list
    ...
    out_file_name = "generation_pythia.jsonl" if args.augmentation_run is None else "generation_pythia_with_RAG_top10_shuffle.jsonl"
